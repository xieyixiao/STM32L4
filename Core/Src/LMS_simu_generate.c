...
static void MedianFilterCG_resetImpl(dsp_private_MedianFilterCG *obj)
{
  ...
  for (c_index = 0; c_index < 40; c_index++) {
    if ((int32_T)fmodf(40.0F + -(real32_T)c_index, 2.0F) == 0) {
  ...
  real32_T i)
{
  ...
  while ((!exitg1) && (i >= -obj->pMaxHeapLength)) {
    ...
    ind1 = truncf(i / 2.0F) + obj->pMidHeap;
    ind2 = i + obj->pMidHeap;
    if (!(obj->pBuf[(int32_T)obj->pHeap[(int32_T)ind1 - 1] - 1] < obj->pBuf
          [(int32_T)obj->pHeap[(int32_T)ind2 - 1] - 1])) {
      exitg1 = true;
    } else {
      temp = obj->pHeap[(int32_T)ind1 - 1];
      obj->pHeap[(int32_T)ind1 - 1] = obj->pHeap[(int32_T)ind2 - 1];
  ...
  real32_T i)
{
  ...
  while ((!exitg1) && (i <= obj->pMinHeapLength)) {
    ...
    ind1 = i + obj->pMidHeap;
    ind2 = truncf(i / 2.0F) + obj->pMidHeap;
    if (!(obj->pBuf[(int32_T)obj->pHeap[(int32_T)ind1 - 1] - 1] < obj->pBuf
          [(int32_T)obj->pHeap[(int32_T)ind2 - 1] - 1])) {
      exitg1 = true;
    } else {
  ...
  if (p > obj->pMID.pMidHeap) {
    if (vprev < rtu_0) {
      ...
      while ((!exitg1) && (vprev > 0.0F)) {
        flag_tmp_tmp = truncf(vprev / 2.0F);
        ...
        if (!flag) {
          ...
          vprev = truncf(vprev / 2.0F) + obj_0->pMidHeap;
          temp = obj_0->pHeap[(int32_T)p - 1];
          obj_0->pHeap[(int32_T)p - 1] = obj_0->pHeap[(int32_T)vprev - 1];
          obj_0->pHeap[(int32_T)vprev - 1] = temp;
    ...
    if (rtu_0 < vprev) {
      ...
      while ((!exitg1) && (vprev < 0.0F)) {
        flag_tmp_tmp = truncf(vprev / 2.0F);
        flag = (obj_0->pBuf[(int32_T)obj_0->pHeap[(int32_T)(flag_tmp_tmp +
                 obj_0->pMidHeap) - 1] - 1] < obj_0->pBuf[(int32_T)obj_0->pHeap
                [(int32_T)(vprev + obj_0->pMidHeap) - 1] - 1]);
        if (!flag) {
          exitg1 = true;
        } else {
          p = truncf(vprev / 2.0F) + obj_0->pMidHeap;
...
void LMS_simu_generate_step(void)
{
  ...
  if ((rtDW.i_j < 35.0) && (rtb_Delay > rtb_Delay1) && (rtb_Delay >
    ...
    if (rtDW.i_j < 5.0) {
      ...
      rtDW.T_n = fmaxf(rtDW.T_n, rtb_Delay);
      rtDW.N_e = fminf(rtDW.N_e, rtb_Delay);
      rtDW.i_j++;
    }
  }
  ...
  if ((rtDW.i < 30.0) && (rtb_Delay2 > rtb_Delay1) && (rtb_Delay2 >
    ...
    if (rtDW.i < 5.0) {
      ...
      rtDW.T = fmaxf(rtDW.T, rtb_Delay2);
      rtDW.N = fminf(rtDW.N, rtb_Delay2);
      rtDW.i++;
    }
  }
