...
u8 max30102_Bus_Write(u8 Register_Address, u8 Word_Data)
{
	...
	IIC2_Write_Byte(max30102_WR_address | I2C_WR);	/* 此处是写指令 */
	...
	/* 第4步：发送字节地址 */
	IIC2_Write_Byte(Register_Address);
	if (IIC2_Wait_Ack() != 0)
	{
		goto cmd_fail;	/* EEPROM器件无应答 */
	}
...
u8 max30102_Bus_Read(u8 Register_Address)
{
	...
	/* 第4步：发送字节地址， */
	IIC2_Write_Byte((uint8_t)Register_Address);
	if (IIC2_Wait_Ack() != 0)
	{
		goto cmd_fail;	/* EEPROM器件无应答 */
	}
	...
	IIC2_Write_Byte(max30102_WR_address | I2C_RD);	/* 此处是读指令 */
...
u8 max30102_reset(void)
{
    if(max30102_Bus_Write(REG_MODE_CONFIG,0x40)==0) return 1;
    return 0;
}
...
//  IIC_WriteBytes(I2C_WRITE_ADDR, ach_i2c_data, 2);
	IIC2_Write_One_Byte(I2C_WRITE_ADDR,uch_addr,uch_data);
}
...
//   *puch_data=(uint8_t) ch_i2c_data;
	IIC2_Read_One_Byte(I2C_WRITE_ADDR,uch_addr,puch_data);
}
...
void maxim_max30102_read_fifo(uint32_t *pun_red_led, uint32_t *pun_ir_led)
{
  ...
  IIC2_ReadBytes(I2C_WRITE_ADDR,REG_FIFO_DATA,(u8 *)ach_i2c_data,6);
